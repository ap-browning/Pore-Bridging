...
# Create figure
figS7,axs = subplots(4,4,figsize=(6.7,5))

# Load results
@load "Results/Supplementary/Saved/Fisher_MLE_Individual.jld2"
@load "Results/Supplementary/Saved/Fisher_MLE_Combined.jld2"
...
for (i_S,Sᵢ) ∈ enumerate(S), (i_P,L) ∈ enumerate(P)
    ...
    # Plot solution where all pore sizes considered simultaneously
        ...
        sol = SolvePDE(D,λ,K,α,Tfine,L=L,t₀=Fisher_MLE_Combined_Settings[:t₀],u₀=u₀)
...
savefig("Figures/Supplementary/Saved/Fig-S7-Fisher_Fits.pdf")
display(figS7)
