...
#   Fisher_MLE.jl
#
#   Repeat results in MLE.jl for Fisher's equation (α = 0.0)
#
...
# Initialise storage
Fisher_MLE_Individual = [[Dict() for i = 1:length(P)] for i = 1:length(C)]
...
t1 = @elapsed begin
    ...
    @threads for i_P = 1:4
        ...
        for (i_C,Cᵢ) ∈ enumerate(C)
            ...
            # Save results
            push!(Fisher_MLE_Individual[i_C][i_P], :θ̂  => θ̂)
            push!(Fisher_MLE_Individual[i_C][i_P], :L̂  => L̂)
...
# Save settings
Fisher_MLE_Individual_Settings = Dict(
...
# Save
@save "Results/Supplementary/Saved/Fisher_MLE_Individual.jld2" Fisher_MLE_Individual Fisher_MLE_Individual_Settings
...
t2 = @elapsed begin
    ...
    # Save results
    push!(Fisher_MLE_Combined, :θ̂  => θ̂)
    push!(Fisher_MLE_Combined, :L̂  => L̂)

end

# Save settings
Fisher_MLE_Combined_Settings = Dict(
    :β  => β,  :i_β => i_β, :lb₁  => lb₁, :ub₁ => ub₁,
    :θ₀ => θ₀, :t₀  => t₀,  :C   => C,
    :ftol_abs => ftol_abs,  :maxtime => maxtime,
    :walltime => t2)
...
# Save
@save "Results/Supplementary/Saved/Fisher_MLE_Combined.jld2" Fisher_MLE_Combined Fisher_MLE_Combined_Settings
